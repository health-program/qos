        ...
        success: function (res){
            ...
            var archives=rawData.V30002
                for (var j=0;j<archives.length;j++){
                        ...
                        yData.push(((archives[j].publicnum*100)/archives[j].total).toFixed(2))

            }

            var rateRecordsOption = {
                tooltip: {
                    trigger: 'axis',
            ...
            //初始化echarts实例
            var rateRecordsID = echarts.init(document.getElementById('rateRecords'));
            rateRecordsID.setOption(rateRecordsOption);
            var index = 0; //播放所在下标
            var mTime = setInterval(function() {
                rateRecordsID.dispatchAction({
                    type: 'showTip',
                    seriesIndex: 0,
                    dataIndex: index
                });
