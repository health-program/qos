$(function(){
    ...
    $.ajax({
        type : "post",
        url : "http://10.9.1.41:18081/home/page/qos/pressureAndSugar",//请求的 URL地址
          ...
          var dataArray =   Array.from(new Set(newArray))
            ...
            dataArray.forEach(function(val, index, arr){
              for(var i=0;i<arrV3004.length;i++){
                    html+='<tr class="hei8" style="border-bottom:1px solid red">';
                    html+='<td>'+arrArea[i]+'</td>';
                    html+='<td>'+arrV3004Total[i]+'</td>';
                    html+='<td>'+arrV3004[i]+'</td>';
                    html+='<td>'+arrV30009[i]+'</td>';
                    html+='<td>'+arrV3005Total[i]+'</td>';
                    html+='<td>'+arrV3005[i]+'</td>';
                    html+='<td>'+arrV30010[i]+'</td>';
                    html+='</tr>';
                  }
           });
     $("#tbody").append(html)
